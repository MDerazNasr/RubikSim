...
Mesh::Mesh(const std::vector<Vertex> &vertices,
  ...
  //&vertexArray_means pass the address where opengl should write the generated
  //ID
  ...
  // Imp opengl detail - the EBO binding is stored inside the currently bound
  // VAO, that is why we bind the VAO first
   ...
   * Stride means how many bytes OpenGL must jump to get from one vertex to the
   * next. reinterpret_cast<void*> This converts the byte offset into the
   * pointer type OpenGL expects. This looks strange because OpenGL is a C API.
   * When a VBO is bound, the final argument is treated as a byte offset, not a
   * real CPU pointer.
   */
  ...
  /*
   * This describes shader input location 1.
   * For us, location 1 now means normal.
   * nx, ny, nz tell the shader which direction this face points.
   */
...
Mesh::Mesh(Mesh &&other) noexcept
    : vertexArray_(other.vertexArray_), vertexBuffer_(other.vertexBuffer_),
      indexBuffer_(other.indexBuffer_), indexCount_(other.indexCount_) {
...
void Mesh::draw() const {
   ...
   * GL_TRIANGLES means every 3 indices form one triangle
   * static_cast<GLsizei>(indexCount_) gives opengl the number of indices
   * GL_UNSIGNED_INT means each index is an unsigned integer
   * nullptr means start reading indices at the start of the EBO
   */
